...
suite("Asset generation: csproj", () => {
    ...
    test("Create launch.json for project opened in workspace", () => {
        ...
        let generator = new assets_1.AssetGenerator(info, createMockWorkspaceFolder(rootPath));
        generator.setStartupProject(0);
        let launchJson = jsonc_parser_1.parse(generator.createLaunchJsonConfigurations(assets_1.ProgramLaunchType.Console), undefined, { disallowComments: true });
    ...
    test("Create launch.json for Blazor web assembly standalone project opened in workspace", () => {
        ...
        const generator = new assets_1.AssetGenerator(info, createMockWorkspaceFolder(rootPath));
        generator.setStartupProject(0);
        const launchJson = jsonc_parser_1.parse(generator.createLaunchJsonConfigurations(assets_1.ProgramLaunchType.BlazorWebAssemblyStandalone), undefined, { disallowComments: true });
    ...
    test("Create launch.json for nested Blazor web assembly standalone project opened in workspace", () => {
        ...
        let generator = new assets_1.AssetGenerator(info, createMockWorkspaceFolder(rootPath));
        generator.setStartupProject(0);
        let launchJson = jsonc_parser_1.parse(generator.createLaunchJsonConfigurations(assets_1.ProgramLaunchType.BlazorWebAssemblyStandalone), undefined, { disallowComments: true });
        const blazorLaunchConfig = launchJson[0];
    ...
    test("Create launch.json for Blazor web assembly hosted project opened in workspace", () => {
        ...
        const generator = new assets_1.AssetGenerator(info, createMockWorkspaceFolder(rootPath));
        generator.setStartupProject(0);
        const launchJson = jsonc_parser_1.parse(generator.createLaunchJsonConfigurations(assets_1.ProgramLaunchType.BlazorWebAssemblyHosted), undefined, { disallowComments: true });
    ...
    test("Create launch.json for nested Blazor web assembly hosted project opened in workspace", () => {
        ...
        let generator = new assets_1.AssetGenerator(info, createMockWorkspaceFolder(rootPath));
        generator.setStartupProject(0);
        let launchJson = jsonc_parser_1.parse(generator.createLaunchJsonConfigurations(assets_1.ProgramLaunchType.BlazorWebAssemblyHosted), undefined, { disallowComments: true });
        const hostedBlazorLaunchConfig = launchJson[0];
        const programPath = hostedBlazorLaunchConfig.program;
        const cwd = hostedBlazorLaunchConfig.cwd;
        const hosted = hostedBlazorLaunchConfig.hosted;
    ...
    test("Create launch.json for web project opened in workspace", () => {
        ...
        let generator = new assets_1.AssetGenerator(info, createMockWorkspaceFolder(rootPath));
        generator.setStartupProject(0);
        let launchJson = jsonc_parser_1.parse(generator.createLaunchJsonConfigurations(assets_1.ProgramLaunchType.Web), undefined, { disallowComments: true });
        let programPath = launchJson[0].program;
        // ${workspaceFolder}/bin/Debug/netcoreapp1.0/testApp.dll
        let segments = programPath.split(path.posix.sep);
        segments.should.deep.equal(['${workspaceFolder}', 'bin', 'Debug', 'netcoreapp1.0', 'testApp.dll']);
    ...
    test("Add a new item to JSON", () => {
        ...
        const updated = assets_1.updateJsonWithComments(JSON.stringify(original), [newItem], 'configurations', 'name', /*formattingOptions*/ null);
    ...
    test("Update item in JSON", () => {
        ...
        const updated = assets_1.updateJsonWithComments(JSON.stringify(original), [updatedItem], 'configurations', 'name', /*formattingOptions*/ null);
        const parsed = jsonc.parse(updated);
        const configurations = parsed.configurations;
    ...
    test("Update JSON and preserve all comments", () => {
        ...
        const updatedItem = { name: 'build', command: 'dotnet' };
        const updated = assets_1.updateJsonWithComments(original, [updatedItem], 'configurations', 'name', /*formattingOptions*/ null);
    ...
    test("Replace items named OS-COMMENTxxx with JSON comment syntax", () => {
        ...
        let updated = assets_1.replaceCommentPropertiesWithComments(original);
    ...
    test("createLaunchJsonConfigurationsArray removes comments", () => {
        ...
        let generator = new assets_1.AssetGenerator(info, createMockWorkspaceFolder(rootPath));
        generator.setStartupProject(0);
        let launchConfigurations = generator.createLaunchJsonConfigurationsArray(assets_1.ProgramLaunchType.Web);
...
function createMSBuildWorkspaceInformation(projectPath, assemblyName, targetFrameworkShortName, isExe = true, isWebProject = false, isBlazorWebAssemblyStandalone = false, isBlazorWebAssemblyHosted = false) {
    return {
        MsBuild: {
            SolutionPath: '',
            Projects: [
